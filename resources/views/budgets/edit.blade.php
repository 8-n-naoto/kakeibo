...
@section('title', '予算の編集 | 家計簿アプリ')

@section('content')
    <h1 class="text-xl font-bold mb-6">予算を編集する</h1>

    <form method="POST" action="{{ route('budgets.update', $budget) }}" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-xl">
        @csrf @method('PUT')
