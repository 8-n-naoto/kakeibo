...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div class="text-sm text-slate-500">エンゲル係数(食費/支出)</div>
            @if (is_null($engelCoefficient))
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            <div class="grid grid-cols-2 gap-4 mb-3">
                @foreach ([['収入', $yearOverYear['income'], 'emerald'], ['支出', $yearOverYear['expense'], 'rose']] as [$label, $data, $tone])
                    <div>
                        <div class="text-xs text-slate-500">{{ $label }}</div>
                        <div class="text-lg font-bold text-{{ $tone }}-600">¥{{ number_format($data['current']) }}</div>
