...
class BudgetTest extends TestCase
{
    ...
    public function test_上書きを選べば同じ対象と月で登録し直せる(): void
    {
        $payload = ['category_id' => null, 'month' => '2026-08', 'amount' => 200000];

        $this->post(route('budgets.store'), $payload);
        $this->post(route('budgets.store'), array_merge($payload, ['amount' => 300000, 'overwrite' => '1']));

        $this->assertSame(1, Budget::count());
        $this->assertDatabaseHas('budgets', ['amount' => 300000]);
    }

    public function test_同じ対象と月の予算を黙って上書きしない(): void
    {
        // メモごと置き換わるので、「先月いくらにしていたか」を確認せずに消してしまう
        $payload = ['category_id' => null, 'month' => '2026-08', 'amount' => 200000, 'memo' => '去年より締める'];

        $this->post(route('budgets.store'), $payload);

        $response = $this->from(route('budgets.create'))->post(
            route('budgets.store'),
            ['category_id' => null, 'month' => '2026-08', 'amount' => 300000],
        );

        $response->assertSessionHasErrors('amount');
        $this->assertSame(1, Budget::count());
        $this->assertDatabaseHas('budgets', ['amount' => 200000, 'memo' => '去年より締める']);
    }

    public function test_デフォルト予算も黙って上書きしない(): void
    {
        // month が null の行は whereNull で探さないと見つからない
        $payload = ['category_id' => null, 'month' => null, 'amount' => 200000];

        $this->post(route('budgets.store'), $payload);

        $response = $this->from(route('budgets.create'))->post(
            route('budgets.store'),
            ['category_id' => null, 'month' => null, 'amount' => 300000],
        );

        $response->assertSessionHasErrors('amount');
        $this->assertDatabaseHas('budgets', ['amount' => 200000]);
    }

    public function test_編集で既存とぶつかる対象月にはできない(): void
    {
        // (category_id, month) に unique 制約は張れない（NULL を含む行が重複できる）。
        // 二重に登録されると BudgetService の ->first() がどちらを拾うか分からず、
        // 「直したはずの予算が反映されない」という追えない状態になる
        $this->post(route('budgets.store'), ['category_id' => null, 'month' => null, 'amount' => 40000]);
        $this->post(route('budgets.store'), ['category_id' => null, 'month' => '2026-08', 'amount' => 60000]);

        $default = Budget::whereNull('month')->firstOrFail();

        $response = $this->from(route('budgets.edit', $default))->put(route('budgets.update', $default), [
            'category_id' => null,
            'month' => '2026-08',
            'amount' => 40000,
        ]);
        ...
        $response->assertSessionHasErrors('amount');
        $this->assertNull($default->fresh()->month);
